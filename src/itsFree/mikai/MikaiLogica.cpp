...
static void calculateBlockChecksum(uint8_t block[4], uint8_t blockNum) {
    block[0] = 0xFF - blockNum - (block[3] & 0x0F) - ((block[3] >> 4) & 0x0F) - (block[2] & 0x0F) - ((block[2] >> 4) & 0x0F) - (block[1] & 0x0F) -
               ((block[1] >> 4) & 0x0F);
}
...
static void calculateEncryptionKey(struct mykey_t *key) {
    uint32_t otp = (((uint32_t)(0xFFu - key->srix4k->eeprom[0x06][3]) << 24) | ((uint32_t)(0xFFu - key->srix4k->eeprom[0x06][2]) << 16) |
                    ((uint32_t)(0xFFu - key->srix4k->eeprom[0x06][1]) << 8) | (uint32_t)(0xFFu - key->srix4k->eeprom[0x06][0])) +
                   1u;
    ...
    uint32_t masterKey =
        (uint32_t)(key->srix4k->uid *
                   (uint64_t)((((uint32_t)b18[2] << 24) | ((uint32_t)b18[3] << 16) | ((uint32_t)b19[2] << 8) | (uint32_t)b19[3]) + 1u));
...
bool mikai_read_tag(struct mykey_t *key, Arduino_PN532_SRIX *nfc) {
    ...
    key->srix4k->uid = ((uint64_t)uid_bytes[7] << 56) | ((uint64_t)uid_bytes[6] << 48) | ((uint64_t)uid_bytes[5] << 40) |
                       ((uint64_t)uid_bytes[4] << 32) | ((uint64_t)uid_bytes[3] << 24) | ((uint64_t)uid_bytes[2] << 16) |
...
bool mikai_is_reset(struct mykey_t *key) {
    return key->srix4k->eeprom[0x18][0] == 0x8F && key->srix4k->eeprom[0x18][1] == 0xCD && key->srix4k->eeprom[0x18][2] == 0x0F &&
           key->srix4k->eeprom[0x18][3] == 0x48 && key->srix4k->eeprom[0x19][0] == 0xC0 && key->srix4k->eeprom[0x19][1] == 0x82 &&
...
void mikai_get_info_string(struct mykey_t *key, char *out, size_t outLen) {
    ...
    if (!lockId) {
        if (!mikai_is_reset(key)) {
            ...
            snprintf(tmp, sizeof(tmp), "Credito: %u.%02u EUR\n", c / 100, c % 100);
            strncat(out, tmp, outLen - strlen(out) - 1);
        }
        snprintf(tmp, sizeof(tmp), "SK: %08" PRIX32 "\n", key->encryptionKey);
        strncat(out, tmp, outLen - strlen(out) - 1);
        snprintf(tmp, sizeof(tmp), "Associata: %s\n", !mikai_is_reset(key) ? "si" : "no");
        strncat(out, tmp, outLen - strlen(out) - 1);
    }
    ...
    int yy = (key->srix4k->eeprom[0x08][3] & 0x0F) * 1000 + ((key->srix4k->eeprom[0x08][3] & 0xF0) >> 4) * 100 +
             ((key->srix4k->eeprom[0x08][2] & 0xF0) >> 4) * 10 + (key->srix4k->eeprom[0x08][2] & 0x0F);
    snprintf(tmp, sizeof(tmp), "Produzione: %02d/%02d/%04d\n", dd, mm, yy);
    strncat(out, tmp, outLen - strlen(out) - 1);
    ...
    for (uint8_t i = 0; i < 8; i++) {
        ...
        if (eb[0] == 0xFF && eb[1] == 0xFF && eb[2] == 0xFF && eb[3] == 0xFF) {
            snprintf(tmp, sizeof(tmp), "[%u] Nessuna transazione\n", i);
        } else {
            uint8_t td = eb[0] >> 3;
...
static bool srix_decrease_block6(struct srix_t *target, uint32_t toDecrease) {
    ...
    uint32_t b6 = ((uint32_t)target->eeprom[0x06][3] << 24) | ((uint32_t)target->eeprom[0x06][2] << 16) | ((uint32_t)target->eeprom[0x06][1] << 8) |
