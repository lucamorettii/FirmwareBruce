...
#include "itsfree/mikai/MikaiMenu.h"
#include <Wire.h>
...
void Mikai::optionsMenu() {
    if (!nfcIsInit) {
        ...
        nfcIsInit = nfc.init();
        if (!nfcIsInit) {
            displayError("Inizializzazione PN532 fallita.", true);
            return;
        }
    }
    ...
    std::vector<Option> options = {
        ...
        {"Aggiungi credito", []() { actionAddCredit(); },    false},
        {"Imposta credito",  []() { actionSetCredit(); },    false},
        {"Reset",            []() { actionReset(); },        false},
        {"Esporta vendor",   []() { actionExportVendor(); }, false},
        {"Importa vendor",   []() { actionImportVendor(); }, false},
        {"Dump",             []() { actionDump(); },         false},
    };
...
void Mikai::drawIcon(float scale) {
    ...
    tft.drawRect(left + padding, panelTop, w - 2 * padding, h - glassHeight - 2 * padding, bruceConfig.priColor);
